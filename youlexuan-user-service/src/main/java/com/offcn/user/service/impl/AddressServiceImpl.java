...
public class AddressServiceImpl implements AddressService {
    ...
    public List<TbProvinces> findAllProvinces() {
        List<TbProvinces> provincesList1 = (List<TbProvinces>) redisTemplate.boundValueOps("provincesList").get();
        if (provincesList1 != null) {
            return provincesList1;
        } else {
            List<TbProvinces> tbProvincesList = provincesMapper.selectByExample(null);
            redisTemplate.boundValueOps("provincesList").set(tbProvincesList);
    ...
    public List<TbCities> byProvinceIdFindCities(String provinceId) {
        List<TbCities> citiesList = (List<TbCities>) redisTemplate.boundHashOps("citiesList").get(provinceId);
        if (citiesList != null) {
            return citiesList;
        } else {
            ...
            List<TbCities> tbCitiesList = citiesMapper.selectByExample(example);
            redisTemplate.boundHashOps("citiesList").put(provinceId, tbCitiesList);
            return tbCitiesList;
        }
    }
    ...
    public List<TbAreas> byCityIdFindAreas(String cityId) {
        List<TbAreas> areasList = (List<TbAreas>) redisTemplate.boundHashOps("areasList").get(cityId);
        if (areasList != null) {
            return areasList;
        } else {
            ...
            List<TbAreas> tbAreasList = areasMapper.selectByExample(example);
            redisTemplate.boundHashOps("areasList").put(cityId, tbAreasList);
            return tbAreasList;
        }
    }
